  ...
  try {
    ...
    for (const cat of categories) {
      try {
        ...
        console.error(
          `✗ Failed to migrate category ${cat.CatName}:`,
          error.message
        );
      }
    }
      ...
      try {
        await prisma.platform.create({
          data: {
            ...
            categoryId: platform.BodyCatID
              ? parseInt(platform.BodyCatID)
              : null,
          },
        });
        ...
        console.error(
          `✗ Failed to migrate platform ${platform.Name}:`,
          error.message
        );
      }
    }
    ...
    for (const product of products) {
      try {
        ...
        if (product.ImageLarge) {
          await prisma.image.create({
            data: {
              ...
              productId: newProduct.id,
              sortOrder: 0,
              altText: `Image for ${product.PartNumber} - ${product.ProductName}`,
            },
          });
        }
          ...
          for (const [index, imgUrl] of additionalImages.entries()) {
            await prisma.image.create({
              data: {
                ...
                productId: newProduct.id,
                sortOrder: index + 1,
                altText: `Additional image ${index + 1} for ${
                  product.ProductName
                }`,
              },
            });
        ...
        console.error(
          `✗ Failed to migrate product ${product.ProductName}:`,
          error.message
        );
      }
    }
    ...
    for (const vehicle of vehicles) {
      try {
        ...
        console.log(
          `✓ Migrated vehicle: ${vehicle.Make} ${vehicle.Model} ${vehicle.StartYear}`
        );
      } catch (error) {
        console.error(
          `✗ Failed to migrate vehicle ${vehicle.Make} ${vehicle.Model}:`,
          error.message
        );
      }
    }
