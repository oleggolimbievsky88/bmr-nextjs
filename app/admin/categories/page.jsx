...
export default function AdminCategoriesPage() {
	...
	const handleEditMainCategory = async (mainCategory) => {
		try {
			const response = await fetch(`/api/admin/maincategories/${mainCategory.MainCatID}`)
			const data = await response.json()
			if (response.ok) {
	...
	const handleCategorySubmit = async (e) => {
		...
		try {
			...
			setSuccess(editingItem ? 'Category updated successfully!' : 'Category created successfully!')
			resetCategoryForm()
			fetchCategories()
	...
	const handleMainCategorySubmit = async (e) => {
		...
		try {
			...
			setSuccess(editingItem ? 'Main category updated successfully!' : 'Main category created successfully!')
		...
		<div className="admin-categories-page">
			<div className="admin-page-header">
				...
				<div className="admin-toolbar">
					...
					>
						+ Add New {activeTab === 'categories' ? 'Category' : 'Main Category'}
					</button>
				</div>
			</div>
				...
				<>
					{showForm && (
						<div className="admin-card">
							<h2 className="mb-4">{editingItem ? 'Edit Category' : 'Add New Category'}</h2>
							<form onSubmit={handleCategorySubmit} className="admin-category-form">
								...
								<div className="admin-form-group">
									...
									<input
										...
										className="form-control"
									/>
									{categoryFormData.CatImage && categoryFormData.CatImage !== '0' && !categoryImage && (
										<div className="mt-2">
											<img 
												src={categoryFormData.CatImage.startsWith('http') ? categoryFormData.CatImage : `/${categoryFormData.CatImage}`} 
												...
												<tr key={category.CatID}>
													...
													<td>
														{category.CatImage && category.CatImage !== '0' ? (
															<img 
																src={category.CatImage.startsWith('http') ? category.CatImage : `/${category.CatImage}`} 
			...
			{activeTab === 'maincategories' && (
				<>
					{showForm && (
						<div className="admin-card">
							<h2 className="mb-4">{editingItem ? 'Edit Main Category' : 'Add New Main Category'}</h2>
							<form onSubmit={handleMainCategorySubmit} className="admin-maincategory-form">
								<div className="admin-form-group">
								...
								<div className="admin-form-group">
									...
									<input
										...
										className="form-control"
									/>
									{mainCategoryFormData.MainCatImage && mainCategoryFormData.MainCatImage !== '0' && !mainCategoryImage && (
										<div className="mt-2">
											<img 
												src={mainCategoryFormData.MainCatImage.startsWith('http') ? mainCategoryFormData.MainCatImage : `/${mainCategoryFormData.MainCatImage}`} 
								...
								<div className="admin-form-actions">
									<button type="submit" className="admin-btn-primary">
										{editingItem ? 'Update Main Category' : 'Create Main Category'}
									</button>
									<button type="button" className="admin-btn-secondary" onClick={resetMainCategoryForm}>
										Cancel
									</button>
								</div>
							</form>
										...
										{mainCategories.length === 0 ? (
											...
											mainCategories.map((mainCategory) => (
												<tr key={mainCategory.MainCatID}>
													...
													<td>
														{mainCategory.MainCatImage && mainCategory.MainCatImage !== '0' ? (
															<img 
																src={mainCategory.MainCatImage.startsWith('http') ? mainCategory.MainCatImage : `/${mainCategory.MainCatImage}`} 
													...
													<td>
														...
														<button
															className="admin-btn-danger"
															onClick={() => handleDeleteMainCategory(mainCategory.MainCatID)}
														>
															Delete
														</button>
													</td>
