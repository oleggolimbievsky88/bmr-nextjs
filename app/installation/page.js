...
export default function InstallationPage() {
	...
	useEffect(() => {
		if (selectedPlatform) {
			const fetchCategories = async () => {
				try {
					const res = await fetch(`/api/installation/categories?platform=${selectedPlatform}`)
					if (res.ok) {
						const data = await res.json()
	...
	const handlePartNumberSearch = async (e) => {
		...
		try {
			const res = await fetch(`/api/installation/search?partNumber=${encodeURIComponent(partNumber.trim())}`)
			if (res.ok) {
				const data = await res.json()
	...
	const handlePlatformCategorySearch = async () => {
		...
		setPartNumber('') // Clear part number when searching by platform
		try {
			const res = await fetch(`/api/installation/products?platform=${selectedPlatform}${selectedCategory ? `&category=${selectedCategory}` : ''}`)
			if (res.ok) {
				const data = await res.json()
				setProducts(data.products || [])
		...
		<div>
				...
				<div className="installation-hero-bg">
					...
					<div className="installation-hero-content">
						<div className="container">
							<h1 className="installation-hero-title">INSTALLATION<br />INSTRUCTIONS</h1>
						</div>
					</div>
				</div>
			</section>
			...
			<div className="installation-content-wrapper">
				<div className="container py-5">
					...
					<div className="row mb-5">
						...
						<div className="col-md-6 mb-4">
							<div className="installation-search-section">
								<h3 className="installation-section-title">Search by Part Number:</h3>
						...
						<div className="col-md-6 mb-4">
							<div className="installation-search-section">
								<h3 className="installation-section-title">Search by Platform:</h3>
								<div className="row mt-3">
									<div className="col-md-6 mb-2">
										...
										>
											...
											{platforms.map((platform) => (
												<option key={platform.id || platform.BodyID} value={platform.id || platform.BodyID}>
													{platform.startYear && platform.startYear !== '0' && platform.endYear && platform.endYear !== '0'
									...
									<div className="col-md-6 mb-2">
										<select
											...
											disabled={loading || !selectedPlatform || categories.length === 0}
										>
											<option value="">Search Categories...</option>
											{categories.map((category) => (
												<option key={category.id || category.CatID} value={category.id || category.CatID}>
					...
					{!loading && products.length === 0 && (selectedPlatform || partNumber) && (
						<div className="row mb-5">
							<div className="col-12">
