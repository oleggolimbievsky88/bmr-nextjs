...
export default async function GMPlatformsPage() {
  ...
  const lateModel = platforms.filter((p) =>
    p.bodyCatName?.includes("GM Late Model")
  );
  const midMuscle = platforms.filter((p) =>
    p.bodyCatName?.includes("GM Mid Muscle")
  );
  const classicMuscle = platforms.filter((p) =>
    p.bodyCatName?.includes("GM Classic Muscle")
  );
    ...
    <div className="container pt-3 pb-5" style={{ minHeight: "60vh" }}>
      <div className="text-center">
        <h1
          ...
          style={{ lineHeight: "40px", marginTop: "30px", fontSize: "2.5rem" }}
        >
          GM Platforms
        </h1>
      </div>
