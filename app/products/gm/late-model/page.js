...
export default async function GMLateModelPage() {
  const platforms = await getGMPlatformsWithVehicles();
  const list = platforms.filter((p) => p.bodyCatName?.includes("GM Late Model"));

  return (
    <div className="container pt-3 pb-5" style={{ minHeight: "60vh" }}>
      <div className="text-center">
        <h1 className="home-title d-inline-block" style={{ lineHeight: "30px", marginTop: "30px" }}>GM Late Model Cars</h1>
      </div>
      <p className="text-center text_black-2 mt-1 mb-5">Explore late-model GM platforms we support.</p>
