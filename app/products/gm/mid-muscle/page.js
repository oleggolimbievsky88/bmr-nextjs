...
export default async function GMMidMusclePage() {
  const platforms = await getGMPlatformsWithVehicles();
  const list = platforms.filter((p) => p.bodyCatName?.includes("GM Mid Muscle"));

  return (
    <div className="container pt-3 pb-5" style={{ minHeight: "60vh" }}>
      <div className="text-center">
        <h1 className="home-title d-inline-block" style={{ lineHeight: "30px", marginTop: "30px" }}>GM Mid Muscle Cars</h1>
      </div>
      <p className="text-center text_black-2 mt-1 mb-3">Explore mid-muscle GM platforms we support.</p>
