...
export function generateOrderConfirmationHTML(orderData) {
  ...
  // Prefer production domain so confirmation email links don't point to Vercel.
  // Set NEXT_PUBLIC_SITE_URL=https://bmrsuspension.com (or your live domain) in production.
  const baseUrl =
    ...
    <body>
      <div class="email-wrapper">
        <div class="email-container">
                    ...
                    <tr>
                      <td class="product-cell">
                        <div class="product-info">
                          ${(function () {
                            const resolved = getProductImageUrl(item.image);
                            if (!resolved) return "";
                            const imgSrc = resolved.startsWith("http")
                              ? resolved
                              : `${base}${resolved}`;
                            return `<img src="${imgSrc}" alt="${(item.name || "").replace(/"/g, "&quot;")}" class="product-image" width="48" height="48" style="width:48px; height:48px; object-fit:cover; border-radius:8px; margin-right:12px;" />`;
                          })()}
                      ...
                      <td>${
                        ...
                        item.color === "Default" ||
                        /gift\s*certificate/i.test(item.name || "") ||
                        /^gc\d+/i.test(item.partNumber || "")
                          ? ""
                          : `<span class="color-badge ${getEmailColorClass(item.color)}">${item.color}</span>`
                      }</td>
                      <td>${
                      ...
                      <td class="text-end" style="font-weight: bold;">$${(
                        ...
                        (item.lineDiscount ?? 0) > 0
                          ? `<br/><span style="color: #198754; font-size: 12px;">Coupon: -$${Number(item.lineDiscount).toFixed(2)}</span>`
                          : ""
                      }</td>
                    </tr>
                  `,
          ...
          ${
            orderData.giftCards && orderData.giftCards.length > 0
              ? `
          <div class="card" style="border: 2px solid #dc3545;">
            <div class="card-header" style="background: #fff5f5;">
              <h3 style="margin: 0;">🎁 Gift Card Code(s)</h3>
              <small>Save these codes — they can be used at checkout until the balance is used.</small>
            </div>
            <div class="card-body">
                ...
                <tbody>
                  ${orderData.giftCards
                    .map(
                      (gc) => `
                    <tr>
                      <td><code class="part-number" style="font-size: 14px;">${gc.code}</code></td>
                      <td class="text-end">$${parseFloat(gc.initial_amount || 0).toFixed(2)}</td>
                      <td class="text-end">$${parseFloat(gc.remaining_balance || gc.initial_amount || 0).toFixed(2)}</td>
                    </tr>
                  `,
                    )
                    .join("")}
