...
export async function createUser(userData) {
  ...
  try {
    const passwordPlain = password_a ?? password;
    const hashedPassword = await hashPassword(password);
    const [result] = await pool.query(
      `INSERT INTO customers
			(email,password_a, password, firstname, lastname, role, dealerTier, dealerDiscount, datecreated, address2, shippingaddress2)
			VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW(), '', '')`,
      [
        email,
        passwordPlain,
