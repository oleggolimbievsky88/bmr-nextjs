...
export async function updateBrand(key, data) {
  ...
  const optionalJsonColumns = ["nav_platform_ids", "homepage_sections"]; // may be missing on older DBs
  ...
  try {
    ...
    const isHomepageSections =
      err.message && String(err.message).includes("homepage_sections");
    if (
      isUnknownColumn &&
      (isNavPlatformIds || isHomepageSections) &&
