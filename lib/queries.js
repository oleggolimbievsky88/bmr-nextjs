...
// Get menu data - prefer platforms table, fall back to bodies
export async function getMenuData() {
  try {
    ...
    try {
      ...
      console.warn(
        "Menu: platforms query failed, falling back to bodies:",
        err.message,
      );
    }
    if (rows.length === 0) {
      try {
        const [bodyRows] = await pool.query(`
          SELECT b.BodyID, b.Name, b.StartYear, b.EndYear, b.BodyOrder, b.Image,
                 bc.BodyCatID, bc.BodyCatName, bc.Position
          FROM bodies b
        ...
        console.warn("Menu: bodies query failed:", bodyErr.message);
      }
    }
...
// Slug must match getPlatformBySlug: prefer platforms.slug, else buildPlatformSlugFromRow(body).
export async function getFordPlatformsWithVehicles() {
  try {
    const query = `
      ...
      GROUP BY b.BodyID, b.Name, b.StartYear, b.EndYear, b.Image, b.HeaderImage, p.slug
...
export async function getGMPlatformsWithVehicles() {
  try {
    const query = `
      ...
      GROUP BY b.BodyID, b.Name, b.StartYear, b.EndYear, b.Image, b.HeaderImage, p.slug, bc.BodyCatName
      ORDER BY b.BodyOrder
    `;
    const [rows] = await pool.query(query);
    return rows.map((row) => {
...
export async function getProductById(productId) {
      ...
      .reduce((acc, imgSrc, index, array) => {
        ...
        if (index % 2 === 0 && index + 1 < array.length) {
          ...
          if (smallImg !== "0" && largeImg !== "0") {
            acc.push({
              imgSrc: `http://legacy.bmrsuspension.com/siteart/products/${largeImg}`,
              smallImgSrc: `http://legacy.bmrsuspension.com/siteart/products/${smallImg}`,
  ...
  const mainImage =
    product.ImageLarge && product.ImageLarge.trim() !== "0"
      ? {
          imgSrc: `http://legacy.bmrsuspension.com/siteart/products/${product.ImageLarge.trim()}`,
          smallImgSrc: `http://legacy.bmrsuspension.com/siteart/products/${product.ImageLarge.trim()}`,
...
// Get platform by slug: prefer platforms table, then fall back to bodies.
export async function getPlatformBySlug(slug) {
  ...
  try {
    ...
    // platforms table missing; fall through to bodies
  }

  // 2) Fall back to bodies (e.g. ColdFusion-added rows that only exist in bodies)
  const [bodyRows] = await pool.query(
  ...
  for (const row of bodyRows) {
    const rowSlug = buildPlatformSlugFromRow(row);
    if (rowSlug === normalizedSlug) {
      return toPlatformShape(row, rowSlug);
    }
  }
...
/**
 * Fetch a platform by ID: prefer platforms table, then bodies.
 * id is PlatformID or BodyID (used as BodyID in product/maincategory queries).
 */
export async function getPlatformById(id) {
  ...
  return toPlatformShape(bodyRows[0], buildPlatformSlugFromRow(bodyRows[0]));
}
...
// Coupons apply ONLY to individual BMR Suspension products (excludes non-BMR, scratch & dent, packages, merchandise, low margin), lower 48 US.
export async function validateCouponForCart(
  ...
  try {
    ...
    // Coupons apply ONLY to individual BMR Suspension products (excludes non-BMR, scratch & dent, packages, merchandise, low margin)
    const couponEligible = await areAllProductsCouponEligible(productIds);
    if (!couponEligible) {
      return {
        valid: false,
        message:
          "This coupon only applies to individual BMR Suspension products. It does not apply to other brands, scratch & dent, packages, merchandise, or low-margin items.",
      };
    }
 ...
 * BMR products only; includes scratch & dent (BlemProduct). Excludes: non-BMR,
 * BMR packages, BMR merchandise, low margin. Lower 48 is enforced at the
 * shipping-rates layer. Note: couponexempt/scratch & dent get free shipping;
 * they are only excluded from coupon discounts.
 * @param {number[]} productIds - Product IDs in the cart.
 * @returns {Promise<boolean>}
 */
