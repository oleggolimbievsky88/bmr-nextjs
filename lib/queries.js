...
export async function getCategoriesByPlatform(platformSlug, mainCategory = null) {
  // First get the BodyID from the platform slug
  const platformQuery = `
    SELECT BodyID, Name, StartYear, EndYear, HeaderImage
    FROM bodies 
    WHERE slug = ?
  `;
  ...
  const categories = rows.map(({ headerImage, platformName, startYear, endYear, ...category }) => category);
...
export async function getProductsByMainCategory(platformSlug, mainCategory, limit = null) {
  let query = `
    SELECT DISTINCT 
      p.*,
      c.CatName as categoryName,
  ...
  const params = limit ? [platformSlug, mainCategory, limit] : [platformSlug, mainCategory];
  const [rows] = await pool.query(query, params);
  return rows;
}
