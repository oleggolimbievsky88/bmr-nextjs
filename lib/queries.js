...
export async function getFilteredProductsPaginated({
  ...
  // Normalize IDs (MySQL driver may return BigInt; avoid NaN)
  ...
  // Product is on this platform if: in product_platforms, or legacy p.BodyID, or in product_platform_category (multi-platform feature)
  ...
  if (
    ...
    // Product is included if: (1) it has a meaningful app year range that contains applicationYear,
    // or (2) it has no meaningful range (empty, null, or '0'/'0') and the platform's year range contains applicationYear.
    // Treat StartAppYear/EndAppYear of '' or '0' as "use platform range" (schema default is '0').
    where.push(
      `(
  ...
  // Main category filter: product must be in at least one category under this main category.
  // Use EXISTS (no JOIN on categories) so we match via product_platform_category or legacy p.CatID.
