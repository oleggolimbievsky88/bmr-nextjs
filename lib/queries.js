  ...
  try {
    ...
    try {
      ...
      try {
        ...
        if (columnError.code === "ER_BAD_FIELD_ERROR" || columnError.code === 1054) {
      ...
      try {
        ...
        if (subtotalError.code === "ER_BAD_FIELD_ERROR" || subtotalError.code === 1054) {
...
export async function createOrderItems(orderId, items) {
  try {
    ...
    console.log(`Successfully created ${results.length} order items for order ${orderId}`);
    return results;
  } catch (error) {
...
// Admin: Get all orders with items count
export async function getAllOrdersAdmin(limit = 100, offset = 0, status = null) {
  try {
    let sql = `
      SELECT 
        o.*,
    ...
    sql += ` GROUP BY o.new_order_id ORDER BY o.order_date DESC LIMIT ? OFFSET ?`
...
export async function getAllCustomersAdmin(limit = 100, offset = 0, search = null) {
  try {
    let sql = `
      SELECT 
    ...
    sql += ` ORDER BY datecreated DESC LIMIT ? OFFSET ?`
    params.push(limit, offset)
...
export async function getAllCouponsAdmin() {
  try {
    ...
    return rows.map(row => ({
      ...row,
      customer_segments: row.customer_segments ? JSON.parse(row.customer_segments) : null,
      product_categories: row.product_categories ? JSON.parse(row.product_categories) : null,
      excluded_products: row.excluded_products ? JSON.parse(row.excluded_products) : null,
    }))
  } catch (error) {
...
export async function createCouponAdmin(couponData) {
  try {
    ...
    const values = [
      ...
      couponData.customer_segments ? JSON.stringify(couponData.customer_segments) : null,
      couponData.product_categories ? JSON.stringify(couponData.product_categories) : null,
      couponData.excluded_products ? JSON.stringify(couponData.excluded_products) : null,
...
export async function getAllProductsAdmin(limit = 100, offset = 0, search = null) {
  try {
    let sql = `SELECT * FROM products WHERE 1=1`
    const params = []
    
    if (search) {
      sql += ` AND (ProductName LIKE ? OR PartNumber LIKE ?)`
      const searchTerm = `%${search}%`
      params.push(searchTerm, searchTerm)
    }
    
    sql += ` ORDER BY ProductID DESC LIMIT ? OFFSET ?`
    params.push(limit, offset)
