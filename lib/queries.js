...
import { navIdFromPlatformGroupName } from "./navPlatformId";
import {
  normalizeAttributeName,
  slugifyAttributeName,
} from "./attributeHelpers";
import { getAcPanelPowderCoatUnitPrice } from "./acPanelPowderCoat";
import { expandPartNumberAliases } from "./partNumberSearch";
...
export async function getDealerPOWithItems(poId, customerId = null) {
  ...
  const [items] = await pool.query(
    `SELECT d.id, d.po_id, d.product_id, d.part_number, d.product_name, d.quantity,
            d.color_id, d.color_name, d.unit_price, d.created_at,
            d.grease_id, d.grease_name, d.anglefinder_id, d.anglefinder_name, d.hardware_id, d.hardware_name,
            d.hardware_pack_ids,
            col.ColorPrice AS color_price
     FROM dealer_po_items d
     LEFT JOIN colors col ON col.ColorID = d.color_id
     WHERE d.po_id = ? ORDER BY d.id ASC`,
    [poId],
  );
...
/** List past orders for a dealer by customer_id. */
...
// Uses platform_groups so platforms added in admin (BodyCatID = platform_groups.id) show in the mega menu.
// Driven by brand navPlatformIds: only nav items marked "Dropdown (mega menu)" in admin get platform lists.
// Platform group → nav id is derived from group name (see navIdFromPlatformGroupName); no hardcoded groups.
export async function getMenuData(brand) {
  try {
    const navPlatformIds = Array.isArray(brand?.navPlatformIds)
      ? brand.navPlatformIds
      : [];
    if (navPlatformIds.length === 0) {
      return {};
    }

    let platformRows;
    try {
      [platformRows] = await pool.query(`
        SELECT p.PlatformID AS BodyID, p.Name, p.StartYear, p.EndYear,
               p.Image, p.slug, p.PlatformOrder AS BodyOrder,
               pg.id AS BodyCatID, pg.name AS BodyCatName
        FROM platforms p
    ...
    // Group platform rows by nav id. Match by: (1) platform group name → nav id (navIdFromPlatformGroupName), (2) nav item label matches group name for nav ids with no platforms yet
    const byNavId = {};
    for (const id of navPlatformIds) {
      byNavId[id] = [];
    }
    const navLabels = brand?.navLabels || {};
    for (const body of rows) {
      let navId = navIdFromPlatformGroupName(body.BodyCatName);
      if (
        navId == null &&
        String(body.slug || "")
          .trim()
      ...
      if (navId != null && byNavId[navId]) {
        byNavId[navId].push(body);
      }
    }
    // Fallback: nav items with Dropdown checked but no platforms yet — match by label to platform group name (e.g. custom id "link-123" with label "AMC")
    for (const navId of navPlatformIds) {
      if (byNavId[navId].length > 0) continue;
      const label = (navLabels[navId] || "").trim().toLowerCase();
      if (!label) continue;
      for (const body of rows) {
        const groupName = (body.BodyCatName || "").trim().toLowerCase();
        if (
          groupName === label ||
          groupName.includes(label) ||
          label.includes(groupName)
        ) {
          byNavId[navId].push(body);
        }
      }
    ...
    const menuData = {};
    for (const id of navPlatformIds) {
      menuData[`${id}Links`] = buildPlatformList(byNavId[id] || []);
    }
    return menuData;
  } catch (error) {
...
export async function getVehiclesForProduct(product) {
  ...
  if (pid) {
    try {
      const [fitRows] = await pool.query(
        `SELECT VehicleID, included, fit_start_year, fit_end_year
         FROM product_vehicle_fitment WHERE ProductID = ?`,
        [pid],
      );
      for (const fr of fitRows || []) {
        fitmentMap[String(fr.VehicleID)] = {
          included: fr.included !== 0,
          fit_start_year: fr.fit_start_year,
          fit_end_year: fr.fit_end_year,
        };
      }
    } catch (e) {
  ...
  for (const v of vehicles) {
    const fit = fitmentMap[String(v.VehicleID)];
    if (fit && fit.included === false) continue;
    ...
    const appEnd = useProductYears
      ...
      : plat
        ...
        : 9999;
    let vStart = parseInt(String(v.StartYear).trim(), 10) || 0;
    let vEnd = parseInt(String(v.EndYear).trim(), 10) || 9999;
    if (fit && (fit.fit_start_year != null || fit.fit_end_year != null)) {
      if (fit.fit_start_year != null) {
        const n = parseInt(String(fit.fit_start_year).trim(), 10);
        if (Number.isFinite(n)) vStart = n;
      }
      if (fit.fit_end_year != null) {
        const n = parseInt(String(fit.fit_end_year).trim(), 10);
        if (Number.isFinite(n)) vEnd = n;
      }
    }
...
export async function getProductById(productId) {
  ...
  const normalizeImagePath = (path) => {
    ...
    if (base.startsWith("http")) {
      try {
        base = new URL(base).pathname || base;
      } catch {
        // keep raw base
      }
    }
    base = base.replace(/\/(small|large)\//gi, "/");
    const extMatch = base.match(/(\.[a-z0-9]+)$/i);
    const ext = extMatch ? extMatch[1] : "";
    const baseName = ext ? base.slice(0, -ext.length) : base;
    const withoutSize = baseName.replace(
      /(_small|-small|_sm|_large|-large|_lg)$/i,
      "",
    );
    return `${withoutSize}${ext}`.toLowerCase();
  };

  const hasSmallSuffix = (path) =>
    typeof path === "string" &&
    /(_small|-small|_sm)(?=\.[a-z0-9]+$)/i.test(path.trim());

  const hasLargeSuffix = (path) =>
    typeof path === "string" &&
    /(_large|-large|_lg)(?=\.[a-z0-9]+$)/i.test(path.trim());

  const imageVariantRank = (path) => {
    if (hasSmallSuffix(path)) return 1;
    if (hasLargeSuffix(path)) return 2;
    return 3;
  };
  ...
  const parseImages = (imagesString, mainImagePath) => {
    if (
      !imagesString ||
    ...
    const rawPaths = imagesString
      .split(/[,;]/)
      .map((path) => path.trim())
      .filter((path) => path !== "" && path !== "0");

    const normalizedNonSmall = new Set();
    rawPaths.forEach((path) => {
      if (!hasSmallSuffix(path)) {
        normalizedNonSmall.add(normalizeImagePath(path));
      }
    });
    if (mainImagePath && !hasSmallSuffix(mainImagePath)) {
      normalizedNonSmall.add(normalizeImagePath(mainImagePath));
    }

    const filtered = rawPaths.filter((path) => {
      if (hasSmallSuffix(path)) {
        const key = normalizeImagePath(path);
        if (normalizedNonSmall.has(key)) {
          return false;
        }
      }
      return true;
  ...
  const otherImagePaths = parseImages(product?.Images, mainImagePath);

  // Combine the main image with other images (if mainImage exists)
  const combinedPaths = mainImagePath
    ? [mainImagePath, ...otherImagePaths]
    : otherImagePaths;
  const byKey = new Map();
  combinedPaths.forEach((path) => {
    ...
    if (imageVariantRank(path) > imageVariantRank(existing)) {
      byKey.set(key, path);
    }
  });
  const uniquePaths = Array.from(byKey.values());
  const images = uniquePaths.map((path, index) => ({
    ...
    for (const category of categories) {
      ...
      const processedBodies = await Promise.all(
        bodies.map(async (body) => {
          ...
          const isUniversal = String(body.Name || "")
            .trim()
            .toLowerCase()
            .includes("universal");
...
function stripYearPrefix(slug) {
  return (slug || "").replace(/^\d{4}-\d{4}-/, "").replace(/^\d{4}-/, "");
}
...
// Normalize category slugs/names to handle punctuation variants.
...
export async function getPlatformBySlug(slug) {
  ...
  const slugHasYearPrefix = /^\d{4}(-\d{4})?/.test(normalizedSlug);
  ...
  if (!slugHasYearPrefix && normalizedSlug) {
    const prefixMatches = [];
    for (const row of platformRows) {
      const rowSlug =
        (row.slug && row.slug.trim()) || buildPlatformSlugFromRow(row);
      const rowNameOnly = stripYearPrefix(normalizeSlug(rowSlug));
      if (!rowNameOnly) continue;
      if (
        rowNameOnly === normalizedSlug ||
        rowNameOnly.replace(/-/g, "") === normalizedNoHyphens
      ) {
        return toPlatformShape(row, rowSlug);
      }
      if (
        rowNameOnly.startsWith(normalizedSlug) ||
        rowNameOnly.replace(/-/g, "").startsWith(normalizedNoHyphens)
      ) {
        prefixMatches.push({ row, rowSlug });
      }
    }
    if (prefixMatches.length === 1) {
      const match = prefixMatches[0];
      return toPlatformShape(match.row, match.rowSlug);
    }
  }
...
export async function getCategoriesByMainCatId(mainCatId, platformId = null) {
  const safePlatformId =
    platformId != null && Number.isFinite(Number(platformId))
      ? Number(platformId)
      : null;
  const usePlatformFilter = safePlatformId != null;
  const platformFilter = usePlatformFilter
    ? `AND (
        EXISTS (SELECT 1 FROM product_platforms pp WHERE pp.ProductID = p.ProductID AND pp.BodyID = ?)
        OR p.BodyID = ?
        OR EXISTS (SELECT 1 FROM product_platform_category ppc_plat WHERE ppc_plat.ProductID = p.ProductID AND ppc_plat.BodyID = ?)
      )`
    : "";
  const platformFilterHero = usePlatformFilter
    ? `AND (
        EXISTS (SELECT 1 FROM product_platforms pp2 WHERE pp2.ProductID = p2.ProductID AND pp2.BodyID = ?)
        OR p2.BodyID = ?
        OR EXISTS (SELECT 1 FROM product_platform_category ppc_plat2 WHERE ppc_plat2.ProductID = p2.ProductID AND ppc_plat2.BodyID = ?)
      )`
    : "";
  const ppcPlatformClause = usePlatformFilter ? " AND ppc.BodyID = ?" : "";
  const ppcPlatformClauseHero = usePlatformFilter ? " AND ppc2.BodyID = ?" : "";

  const query = `
    SELECT c.*,
      COUNT(DISTINCT p.ProductID) as directCount,
      (
        SELECT COALESCE(
          NULLIF(NULLIF(p2.ImageSmall, ''), '0'),
          NULLIF(NULLIF(p2.ImageLarge, ''), '0')
        )
        FROM products p2
        WHERE p2.Display = 1
          AND p2.EndProduct != 1
          ...
          AND (
            EXISTS (
              SELECT 1 FROM product_platform_category ppc2
              WHERE ppc2.ProductID = p2.ProductID
                AND ppc2.CatID = c.CatID${ppcPlatformClauseHero}
            )
            OR FIND_IN_SET(c.CatID, p2.CatID)
          )
          AND (
            NULLIF(NULLIF(p2.ImageSmall, ''), '0') IS NOT NULL
            OR NULLIF(NULLIF(p2.ImageLarge, ''), '0') IS NOT NULL
          )
        ORDER BY p2.NewPartDate DESC, p2.ProductID DESC
        LIMIT 1
      ) as heroImage
    FROM categories c
    LEFT JOIN products p
      ...
      AND (
        EXISTS (
          SELECT 1 FROM product_platform_category ppc
          WHERE ppc.ProductID = p.ProductID
            AND ppc.CatID = c.CatID${ppcPlatformClause}
        )
        OR FIND_IN_SET(c.CatID, p.CatID)
      )
    WHERE c.MainCatID = ?
  ...
  if (usePlatformFilter)
    params.push(safePlatformId, safePlatformId, safePlatformId);
  if (usePlatformFilter) params.push(safePlatformId);
  if (usePlatformFilter)
    params.push(safePlatformId, safePlatformId, safePlatformId);
  if (usePlatformFilter) params.push(safePlatformId);
  params.push(mainCatId);
  ...
  const categories = (rows || []).map((row) => {
    const directCount = Number(row?.directCount ?? row?.productCount ?? 0);
    return {
      ...row,
      directCount: Number.isFinite(directCount) ? directCount : 0,
      productCount: Number.isFinite(directCount) ? directCount : 0,
    };
  });
  ...
  categories.forEach((cat) => {
    const id = toNumberId(cat?.CatID ?? cat?.id);
    if (id == null) return;
    byId.set(id, cat);
    const parentId = toNumberId(cat?.ParentID);
  ...
  const computeTotal = (id) => {
    ...
    let total = Number(cat?.directCount ?? cat?.productCount ?? 0);
  ...
  categories.forEach((cat) => {
    const id = toNumberId(cat?.CatID ?? cat?.id);
    if (id == null) return;
    cat.productCount = computeTotal(id);
  });
  ...
  const query = `
		SELECT
			...
			(
				...
				FROM products p2
				JOIN categories c2 ON c2.MainCatID = m.MainCatID
				WHERE p2.Display = 1
					AND p2.EndProduct != 1
					AND (
						EXISTS (SELECT 1 FROM product_platforms pp WHERE pp.ProductID = p2.ProductID AND pp.BodyID = ?)
						OR p2.BodyID = ?
					)
					AND (
						EXISTS (SELECT 1 FROM product_platform_category ppc WHERE ppc.ProductID = p2.ProductID AND ppc.BodyID = ? AND ppc.CatID = c2.CatID)
						OR FIND_IN_SET(c2.CatID, p2.CatID)
					)
					AND (
						NULLIF(NULLIF(p2.ImageSmall, ''), '0') IS NOT NULL
						OR NULLIF(NULLIF(p2.ImageLarge, ''), '0') IS NOT NULL
...
function buildProductListWhere({
  ...
  const safePlatformId =
    platformId != null && Number.isFinite(Number(platformId))
      ? Number(platformId)
      : null;
  const safeMainCategoryId =
    mainCategoryId != null && Number.isFinite(Number(mainCategoryId))
      ? Number(mainCategoryId)
      : null;
  if (safePlatformId != null) {
    where.push(
      "(EXISTS (SELECT 1 FROM product_platforms pp WHERE pp.ProductID = p.ProductID AND pp.BodyID = ?) OR p.BodyID = ? OR EXISTS (SELECT 1 FROM product_platform_category ppc_plat WHERE ppc_plat.ProductID = p.ProductID AND ppc_plat.BodyID = ?))",
    );
    params.push(safePlatformId, safePlatformId, safePlatformId);
  }
  if (
    ...
    where.push(
      `(
        (NULLIF(TRIM(p.StartAppYear), '') IS NOT NULL AND NULLIF(TRIM(p.EndAppYear), '') IS NOT NULL
         AND (CAST(NULLIF(TRIM(p.StartAppYear), '') AS UNSIGNED) > 0 OR CAST(NULLIF(TRIM(p.EndAppYear), '') AS UNSIGNED) > 0)
        ...
        ((NULLIF(TRIM(p.StartAppYear), '') IS NULL OR NULLIF(TRIM(p.EndAppYear), '') IS NULL
          OR (CAST(COALESCE(NULLIF(TRIM(p.StartAppYear), ''), '0') AS UNSIGNED) = 0 AND CAST(COALESCE(NULLIF(TRIM(p.EndAppYear), ''), '0') AS UNSIGNED) = 0))
         AND EXISTS (
    ...
    params.push(applicationYear, safePlatformId, applicationYear);
  }
  if (safeMainCategoryId != null) {
    if (safePlatformId != null) {
      where.push(
        `(
          EXISTS (
            SELECT 1 FROM product_platform_category ppc
            JOIN categories c ON c.CatID = ppc.CatID AND c.MainCatID = ?
            WHERE ppc.ProductID = p.ProductID AND ppc.BodyID = ?
          )
          OR EXISTS (
            SELECT 1 FROM categories c
            WHERE c.MainCatID = ? AND FIND_IN_SET(c.CatID, p.CatID) != 0
          )
        )`,
      );
      params.push(safeMainCategoryId, safePlatformId, safeMainCategoryId);
    } else {
      where.push(
        "EXISTS (SELECT 1 FROM categories c WHERE c.MainCatID = ? AND FIND_IN_SET(c.CatID, p.CatID) != 0)",
      );
      params.push(safeMainCategoryId);
    }
  }
    ...
    if (validIds.length) {
      if (safePlatformId != null) {
        if (validIds.length === 1) {
          ...
          validIds.forEach((id) => params.push(safePlatformId, id));
          validIds.forEach((id) => params.push(id, id, safePlatformId));
        }
      } else {
        if (validIds.length === 1) {
          where.push("FIND_IN_SET(?, p.CatID)");
...
export async function getFilteredProductsPaginated({
  ...
  const attrSlugs = Object.keys(attributeFilters || {}).filter(
    (k) => Array.isArray(attributeFilters[k]) && attributeFilters[k].length > 0,
  );
  for (const slug of attrSlugs) {
    ...
    where.push(
      `EXISTS (
        SELECT 1 FROM product_attribute_values pav_af
        JOIN category_attributes ca_af ON ca_af.id = pav_af.category_attribute_id
        WHERE pav_af.ProductID = p.ProductID AND ca_af.slug = ? AND pav_af.value IN (${placeholders})
      )`,
    );
  ...
  if (sort === "price-asc") {
    orderBy =
      "CAST(REPLACE(p.Price, ',', '') AS DECIMAL(10,2)) ASC, p.ProductID DESC";
  } else if (sort === "price-desc") {
    orderBy =
      "CAST(REPLACE(p.Price, ',', '') AS DECIMAL(10,2)) DESC, p.ProductID DESC";
  } else if (sort === "newest") {
...
/** Attribute filter options for a category (grouped by attribute, with value counts). Counts reflect the unfiltered category set. */
export async function getAttributeFiltersForCategory({
  ...
  const whereClause = where.length ? "WHERE " + where.join(" AND ") : "";
  const sql = `
    SELECT ca.label AS attributeLabel, ca.slug AS attributeSlug, ca.sort_order,
           pav.value AS value,
           COALESCE(col.ColorName, pav.value) AS label,
           COUNT(DISTINCT p.ProductID) AS count
    FROM products p
    JOIN product_attribute_values pav ON pav.ProductID = p.ProductID
    JOIN category_attributes ca ON ca.id = pav.category_attribute_id
    LEFT JOIN colors col
      ON (
        ca.slug = 'color'
        OR LOWER(ca.label) = 'color'
        OR ca.slug LIKE '%\\_color'
        OR ca.slug LIKE 'color\\_%'
      )
     AND col.ColorID = CAST(pav.value AS UNSIGNED)
    ${whereClause}
    GROUP BY
      ca.id, ca.label, ca.slug, ca.sort_order,
      pav.value,
      COALESCE(col.ColorName, pav.value)
    ORDER BY ca.sort_order, ca.label, COALESCE(col.ColorName, pav.value)
  `;
  const [rows] = await pool.query(sql, params);
  const bySlug = {};
  for (const r of rows || []) {
    if (!bySlug[r.attributeSlug]) {
      bySlug[r.attributeSlug] = {
        attributeLabel: r.attributeLabel,
        attributeSlug: r.attributeSlug,
        values: [],
      };
    }
    bySlug[r.attributeSlug].values.push({
      value: r.value,
      label: r.label,
...
export async function getCategoryIdsBySlugAndMainCat(
  ...
  const decoded = decodeSlug(categorySlug);
  const slug = String(decoded).toLowerCase().trim();
  const query = `
    SELECT CatID
    FROM categories
    WHERE MainCatID = ?
  ...
  if (rows?.length) return (rows || []).map((r) => r.CatID);

  const normalizedSlug = normalizeCategorySlug(decoded);
  if (!normalizedSlug || normalizedSlug === slug) return [];

  const [fallbackRows] = await pool.query(
    `
      SELECT CatID, CatSlug, CatNameSlug, CatName
      FROM categories
      WHERE MainCatID = ?
    `,
    [mainCategoryId],
  ...
  return (fallbackRows || [])
    .filter((row) => {
      return (
        normalizeCategorySlug(row?.CatSlug) === normalizedSlug ||
        normalizeCategorySlug(row?.CatNameSlug) === normalizedSlug ||
        normalizeCategorySlug(row?.CatName) === normalizedSlug
      );
    })
    .map((row) => row.CatID);
}

/** Returns category IDs for a slug restricted to direct children of the main category (ParentID = 0 or NULL). Use for product-type pages so we only show that type, not siblings. */
export async function getCategoryIdsBySlugAndMainCatDirectChild(
  mainCategoryId,
  categorySlug,
) {
  if (!categorySlug || !mainCategoryId) return [];
  ...
  const query = `
    ...
    WHERE MainCatID = ?
      ...
      AND (
        LOWER(TRIM(COALESCE(CatNameSlug, ''))) = ?
        OR LOWER(TRIM(COALESCE(CatSlug, ''))) = ?
        OR LOWER(REPLACE(REPLACE(COALESCE(CatName,''), ' ', '-'), '--', '-')) = ?
      )
    ORDER BY CatID
  ...
  const [fallbackRows] = await pool.query(
    `
      SELECT CatID, CatSlug, CatNameSlug, CatName, ParentID
      FROM categories
      WHERE MainCatID = ?
        AND (ParentID IS NULL OR ParentID = 0)
    `,
...
export async function validateCouponForCart(
  ...
  try {
    ...
    for (const item of cartItems || []) {
      ...
      const powderUnitCart = getAcPanelPowderCoatUnitPrice(
        item?.selectedColor,
        item?.PartNumber ?? item?.partNumber,
      );
      lineTotal += powderUnitCart * qty;
      cartTotal += lineTotal;
    }
...
export async function searchAllQuick(searchQuery, limits = {}) {
  ...
  const partAliases = isPartNumberLike ? expandPartNumberAliases(q) : [];
  const partAliasesWithoutOriginal =
    partAliases.length > 1 ? partAliases.slice(1) : [];
  const aliasContains = partAliasesWithoutOriginal.map((a) => `%${a}%`);
  const aliasStarts = partAliasesWithoutOriginal.map((a) => `${a}%`);
  ...
  // If user searched a part-number-like term with a known suffix (R/H), boost the base part number too
  if (partAliasesWithoutOriginal.length > 0) {
    relevanceCase += partAliasesWithoutOriginal
      .map(() => ` WHEN UPPER(p.PartNumber) = ? THEN 950`)
      .join("");
    relevanceCase += partAliasesWithoutOriginal
      .map(() => ` WHEN UPPER(p.PartNumber) LIKE ? THEN 750`)
      .join("");
  }
  ...
  const wherePartAliasMatch =
    partAliasesWithoutOriginal.length > 0
      ? ` ${partAliasesWithoutOriginal
          .map(() => "OR UPPER(p.PartNumber) LIKE ?")
          .join(" ")}`
      : "";
  ...
  // Bind order MUST match placeholder order in SQL: CASE … END first, then WHERE (…).
  // Previously extra `contains` values shifted bindings so alias PartNumber OR clauses
  // never received `%basePart%` — searches like AA001R failed while AA001 worked.
  let productParams = [
    // CASE: primary PartNumber / ProductName relevance (6)
    exactMatch,
    startsWith,
    contains,
    exactMatch,
    startsWith,
    contains,
    // CASE: stripped color-suffix aliases (e.g. AA001 for AA001R)
    ...(partAliasesWithoutOriginal.length > 0
      ? partAliasesWithoutOriginal
      : []),
    ...(partAliasesWithoutOriginal.length > 0 ? aliasStarts : []),
    ...(hasMultipleWords ? wordPatterns : []),
    makeModelLike,
    makeModelLike,
    ...(vehicleBodyIds.length > 0 ? vehicleBodyIds : []),
    ...(searchYear ? [searchYear] : []),
    // CASE: Description / Features (exactly 2 placeholders)
    contains,
    contains,
    // WHERE: PartNumber contains query + alias contains + text fields
    contains,
    ...(partAliasesWithoutOriginal.length > 0 ? aliasContains : []),
    contains,
    contains,
    contains,
    ...(hasMultipleWords ? wordPatterns : []),
      ...
      ORDER BY
        CASE
          ...
          ${
            partAliasesWithoutOriginal.length > 0
              ? partAliasesWithoutOriginal
                  .map(() => "WHEN UPPER(p.PartNumber) = ? THEN 4")
                  .join("\n          ")
              : ""
          }
          ELSE 5
  ...
  let categoryQuery = `
      ...
      FROM (
        ...
        LEFT JOIN maincategories mc ON c.MainCatID = mc.MainCatID
        WHERE p.Display = 1 AND p.EndProduct != 1 AND (
          UPPER(p.ProductName) LIKE ? OR UPPER(p.PartNumber) LIKE ? ${
            partAliasesWithoutOriginal.length > 0
              ? partAliasesWithoutOriginal
                  .map(() => "OR UPPER(p.PartNumber) LIKE ?")
                  .join(" ")
              : ""
          } OR UPPER(p.Description) LIKE ? OR UPPER(p.Features) LIKE ?
...
export async function getOrderById(orderId, dbPool = pool) {
  try {
    const sql = `
      ...
      LIMIT 1
    `;
    const [rows] = await dbPool.query(sql, [orderId, orderId]);
    return rows[0] || null;
  } catch (error) {
...
export async function getOrderItems(orderId, dbPool = pool) {
  try {
    const sql = `
...
export async function getOrderWithItemsAdmin(orderId, dbPool = pool) {
  try {
    ...
    const [items, tracking_numbers] = await Promise.all([
      getOrderItems(order.new_order_id, dbPool),
...
export async function getOrderStatusHistory(newOrderId, dbPool = pool) {
  try {
...
export async function getOrderCcRevealLog(newOrderId, dbPool = pool) {
  try {
    const [rows] = await dbPool.query(
...
export async function getOrderTrackingNumbers(newOrderId, dbPool = pool) {
  try {
    const [rows] = await dbPool.query(
...
export async function deleteOrderTrackingNumber(id, dbPool = pool) {
  try {
    const [result] = await dbPool.query(
...
export async function updateCustomerProfileAdmin(customerId, profileData) {
  try {
    ...
    if (profileData.password_a !== undefined) {
      updates.push("password_a = ?");
      values.push(profileData.password_a);
    }
...
export async function getProductByIdAdmin(productId) {
  try {
    ...
    try {
      const [fitRows] = await pool.query(
        `SELECT VehicleID, included, fit_start_year AS fitStartYear, fit_end_year AS fitEndYear
         FROM product_vehicle_fitment WHERE ProductID = ?`,
        [productId],
      );
      for (const r of fitRows || []) {
        vehicleFitment[String(r.VehicleID)] = {
          included: r.included !== 0,
          fitStartYear:
            r.fitStartYear != null && r.fitStartYear !== ""
              ? String(r.fitStartYear)
              : "",
          fitEndYear:
            r.fitEndYear != null && r.fitEndYear !== ""
              ? String(r.fitEndYear)
              : "",
        };
      }
    ...
    product.vehicleFitment = vehicleFitment;
...
function _normalizeVehicleFitmentRows(raw) {
  ...
  const rows = [];
  for (const [vid, rec] of Object.entries(obj)) {
    const vehicleId = parseInt(String(vid), 10);
    if (!Number.isFinite(vehicleId) || vehicleId <= 0) continue;
    const included =
      rec &&
      rec.included !== false &&
      rec.included !== 0 &&
      rec.included !== "0"
        ? 1
        : 0;
    let fs = rec?.fitStartYear ?? rec?.fit_start_year;
    let fe = rec?.fitEndYear ?? rec?.fit_end_year;
    if (fs === "" || fs === undefined || fs === null) fs = null;
    else {
      const n = parseInt(String(fs), 10);
      fs = Number.isFinite(n) ? n : null;
    }
    if (fe === "" || fe === undefined || fe === null) fe = null;
    else {
      const n = parseInt(String(fe), 10);
      fe = Number.isFinite(n) ? n : null;
    }
    rows.push([vehicleId, included, fs, fe]);
  }
  return rows;
}

async function replaceProductVehicleFitment(productId, productData) {
  if (!productId) return;
  const rows = _normalizeVehicleFitmentRows(productData?.vehicleFitment);
  try {
    await pool.query(
      "DELETE FROM product_vehicle_fitment WHERE ProductID = ?",
      [productId],
    );
    if (rows.length === 0) return;
    const vals = rows.map((r) => [productId, r[0], r[1], r[2], r[3]]);
    await pool.query(
      "INSERT INTO product_vehicle_fitment (ProductID, VehicleID, included, fit_start_year, fit_end_year) VALUES ?",
      [vals],
    );
  } catch (e) {
  ...
  try {
    ...
    const sql = `
      INSERT INTO products (
        ...
        hardwarepacks, video, taxexempt, couponexempt, BlemProduct, AttributeCategoryID
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `;
    ...
    if (productId) {
      await replaceProductVehicleFitment(productId, productData);
    }
    return productId;
  } catch (error) {
  ...
  try {
    ...
    const sql = `
      UPDATE products SET
        ...
        taxexempt = ?, couponexempt = ?, BlemProduct = ?, AttributeCategoryID = ?
      WHERE ProductID = ?
    `;
    ...
    const [[rowExists]] = await pool.query(
      `SELECT ProductID FROM products WHERE ProductID = ? LIMIT 1`,
      [productId],
    );
...
// Generate URL-safe slug from main category name (e.g. "Steering & Suspension" -> "steering-suspension")
function mainCategorySlugFromName(name) {
...
export async function createMainCategoryAdmin(mainCategoryData) {
  try {
    const mainCatName = mainCategoryData.MainCatName || "";
    const mainCatSlug =
      mainCategoryData.MainCatSlug || mainCategorySlugFromName(mainCatName);
    const sql = `
...
export async function updateMainCategoryAdmin(mainCatId, mainCategoryData) {
  try {
    const mainCatName = mainCategoryData.MainCatName || "";
    const mainCatSlug =
      mainCategoryData.MainCatSlug != null &&
      mainCategoryData.MainCatSlug !== ""
        ? mainCategoryData.MainCatSlug
        : mainCategorySlugFromName(mainCatName);
    const sql = `
      UPDATE maincategories SET
        BodyID = ?, MainCatImage = ?, MainCatName = ?, MainCatSlug = ?
      WHERE MainCatID = ?
    `;
    const values = [
      mainCategoryData.BodyID || "0",
...
export async function getPlatformGroupsAdmin() {
  try {
    const [rows] = await pool.query(
      "SELECT * FROM platform_groups ORDER BY id ASC",
    );
    return rows || [];
  } catch (e) {
    if (e.code === "ER_NO_SUCH_TABLE") return [];
...
export async function createPlatformGroupAdmin(data) {
  // Keep position DB column as-is; ordering is controlled by brand nav order.
  const [r] = await pool.query(
    "INSERT INTO platform_groups (name) VALUES (?)",
    [data.name || "Group"],
  );
  return r.insertId;
}
...
export async function updatePlatformGroupAdmin(id, data) {
  const [r] = await pool.query(
    // Do not update position here; ordering is controlled by brand nav order.
    "UPDATE platform_groups SET name = ? WHERE id = ?",
    [data.name ?? "", id],
  );
...
export async function getAllManufacturersAdmin() {
  try {
    const [rows] = await pool.query(`
      SELECT * FROM mans ORDER BY ManName ASC, ManID DESC
    `);
    return rows;
  } catch (error) {
...
export async function getManufacturerByIdAdmin(manufacturerId) {
  const [rows] = await pool.query(
    "SELECT * FROM mans WHERE ManID = ? LIMIT 1",
    [manufacturerId],
  );
  return rows?.[0] || null;
}

export async function createManufacturerAdmin(data) {
  const [r] = await pool.query("INSERT INTO mans (ManName) VALUES (?)", [
    data.name ?? data.ManName ?? "0",
  ]);
  return r.insertId;
}

export async function updateManufacturerAdmin(manufacturerId, data) {
  const [r] = await pool.query("UPDATE mans SET ManName = ? WHERE ManID = ?", [
    data.name ?? data.ManName ?? "0",
    manufacturerId,
  ]);
  return r.affectedRows > 0;
...
export async function deleteManufacturerAdmin(manufacturerId) {
  const [r] = await pool.query("DELETE FROM mans WHERE ManID = ?", [
    manufacturerId,
  ]);
  return r.affectedRows > 0;
}
 ...
 * BMR + Control Freak products only.
 * Excludes: non-BMR / non-Control Freak, and (BMR/Control Freak) merchandise.
 * Lower 48 is enforced at the shipping-rates layer.
  ...
  try {
    const [rows] = await pool.query(
      `SELECT COUNT(DISTINCT p.ProductID) AS eligible_count
       ...
       WHERE p.ProductID IN (?)
         AND (
           UPPER(m.ManName) LIKE '%BMR%'
           OR UPPER(m.ManName) LIKE '%B.M.R.%'
           OR UPPER(m.ManName) LIKE '%CONTROL FREAK%'
           OR UPPER(m.ManName) LIKE '%CONTROLFREAK%'
         )
         AND NOT EXISTS (
           SELECT 1 FROM categories c
           LEFT JOIN maincategories mc ON c.MainCatID = mc.MainCatID
...
// ---------------------------------------------------------------------------
// Attribute categories (logical attribute sets: Control Arms, Sway Bars, etc.)
// ---------------------------------------------------------------------------

export async function getAllAttributeCategories() {
  try {
    const [rows] = await pool.query(
      `SELECT id, name, slug, sort_order FROM attribute_categories ORDER BY sort_order ASC, name ASC`,
    );
    return rows || [];
  } catch (error) {
...
export async function getAttributeCategoryById(id) {
  try {
    const [rows] = await pool.query(
      `SELECT id, name, slug, sort_order FROM attribute_categories WHERE id = ?`,
      [id],
    );
    return rows[0] || null;
  } catch (error) {
...
export async function getAttributeCategoryBySlug(slug) {
  try {
    const [rows] = await pool.query(
      `SELECT id, name, slug, sort_order FROM attribute_categories WHERE slug = ?`,
      [slug],
    );
    return rows[0] || null;
  } catch (error) {
    console.error("Error fetching attribute category by slug:", error);
    throw error;
  }
}

export async function findAttributeCategoryByNormalizedName(name) {
  try {
    const normalized = normalizeAttributeName(name);
    const slug = slugifyAttributeName(name);
    if (!normalized && !slug) return null;
    const all = await getAllAttributeCategories();
    return (
      all.find((cat) => {
        if (!cat) return false;
        if (slug && String(cat.slug || "").toLowerCase() === slug) return true;
        return (
          normalized && normalizeAttributeName(cat.name || "") === normalized
        );
      }) || null
    );
  } catch (error) {
    console.error(
      "Error matching attribute category by normalized name:",
      error,
    );
    throw error;
  }
...
export async function getAttributeCategoryAttributeCount(attributeCategoryId) {
  try {
    const [rows] = await pool.query(
      `SELECT COUNT(*) AS cnt FROM category_attributes WHERE attribute_category_id = ?`,
      [attributeCategoryId],
    );
    return rows[0] && rows[0].cnt ? Number(rows[0].cnt) : 0;
  } catch (error) {
    console.error("Error counting category attributes:", error);
    throw error;
  }
}
...
export async function createAttributeCategory(data) {
  try {
    const [result] = await pool.query(
      `INSERT INTO attribute_categories (name, slug, sort_order) VALUES (?, ?, ?)`,
      [
        data.name || "",
        data.slug || "",
        data.sort_order != null ? Number(data.sort_order) : 0,
      ],
    );
    return result.insertId;
  } catch (error) {
...
export async function updateAttributeCategory(id, data) {
  try {
    ...
    values.push(id);
    const [result] = await pool.query(
      `UPDATE attribute_categories SET ${updates.join(", ")} WHERE id = ?`,
      values,
    );
    return result.affectedRows > 0;
  } catch (error) {
...
export async function deleteAttributeCategory(id) {
  try {
    const [result] = await pool.query(
      `DELETE FROM attribute_categories WHERE id = ?`,
      [id],
    );
    return result.affectedRows > 0;
  } catch (error) {
...
// ---------------------------------------------------------------------------
// Category attributes (attribute definitions per attribute category)
// ---------------------------------------------------------------------------

export async function getCategoryAttributesByAttributeCategoryId(
  attributeCategoryId,
) {
  try {
    const [rows] = await pool.query(
      `SELECT id, attribute_category_id, slug, label, type, options, sort_order
...
export async function getCategoryAttributeById(id) {
  try {
    const [rows] = await pool.query(
      `SELECT id, attribute_category_id, slug, label, type, options, sort_order
       FROM category_attributes WHERE id = ?`,
      [id],
    );
    return rows[0] || null;
  } catch (error) {
...
export async function createCategoryAttribute(data) {
  try {
    const [result] = await pool.query(
      `INSERT INTO category_attributes (attribute_category_id, slug, label, type, options, sort_order)
       VALUES (?, ?, ?, ?, ?, ?)`,
      [
        data.attribute_category_id,
        data.slug || "",
        data.label || "",
        data.type || "text",
        data.options != null ? String(data.options) : null,
        data.sort_order != null ? Number(data.sort_order) : 0,
      ],
    );
    return result.insertId;
...
export async function updateCategoryAttribute(id, data) {
  try {
    const updates = [];
    const values = [];
    if (data.slug !== undefined) {
    ...
    if (data.options !== undefined) {
      ...
      values.push(data.options != null ? String(data.options) : null);
    }
    if (data.sort_order !== undefined) {
      updates.push("sort_order = ?");
      values.push(Number(data.sort_order));
    ...
    values.push(id);
    const [result] = await pool.query(
      `UPDATE category_attributes SET ${updates.join(", ")} WHERE id = ?`,
      values,
    );
    return result.affectedRows > 0;
  } catch (error) {
...
export async function getProductAttributeValues(productId) {
  try {
    const [rows] = await pool.query(
      `SELECT
         ...
         ca.type,
         pav.value,
         COALESCE(col.ColorName, pav.value) AS displayValue
       FROM product_attribute_values pav
       JOIN category_attributes ca ON ca.id = pav.category_attribute_id
       INNER JOIN products p
         ON p.ProductID = pav.ProductID
         AND p.AttributeCategoryID IS NOT NULL
         AND ca.attribute_category_id = p.AttributeCategoryID
       LEFT JOIN colors col
         ON (
           ca.slug = 'color'
           OR LOWER(ca.label) = 'color'
        ...
        AND col.ColorID = CAST(pav.value AS UNSIGNED)
       WHERE pav.ProductID = ?
       ORDER BY ca.sort_order ASC, ca.label ASC`,
      [productId],
    );
    return (rows || []).map((r) => ({
...
export async function setProductAttributeValues(
  ...
  if (!attributeCategoryId || !values || typeof values !== "object") return;
  const conn = await pool.getConnection();
  try {
    await conn.query("START TRANSACTION");
    const [attrs] = await conn.query(
      `SELECT id, slug FROM category_attributes WHERE attribute_category_id = ?`,
      [attributeCategoryId],
    );
    ...
    await conn.query(
      `DELETE pav FROM product_attribute_values pav
       JOIN category_attributes ca ON ca.id = pav.category_attribute_id
       WHERE pav.ProductID = ? AND ca.attribute_category_id = ?`,
      [productId, attributeCategoryId],
    );
    const entries = Object.entries(values).filter(
      ([slug, value]) =>
        slug && value !== undefined && value !== null && slugToId.has(slug),
    );
    if (entries.length > 0) {
      const insertRows = entries.map(([slug, value]) => [
        productId,
        slugToId.get(slug),
        String(value),
      ]);
      await conn.query(
        `INSERT INTO product_attribute_values (ProductID, category_attribute_id, value) VALUES ?`,
        [insertRows],
      );
    }
    await conn.query("COMMIT");
