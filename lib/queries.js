...
import { getProductImageUrl } from "./assets";
...
/** Admin: update image (ImageUrl, ImagePosition). */
...
/** Admin: bodies by platform_group_id (or all if no column yet). */
export async function getBodiesByPlatformGroupAdmin(platformGroupId) {
  try {
    const [rows] = await pool.query(
      "SELECT * FROM bodies WHERE platform_group_id = ? ORDER BY BodyOrder ASC, BodyID ASC",
      [platformGroupId],
    );
    return rows || [];
  } catch (e) {
    if (e.code === "ER_BAD_FIELD_ERROR") {
      const [rows] = await pool.query(
        "SELECT * FROM bodies WHERE BodyCatID = ? ORDER BY BodyOrder ASC, BodyID ASC",
        [platformGroupId],
      );
      return rows || [];
    }
...
export async function createBodyAdmin(data) {
  const [r] = await pool.query(
    `INSERT INTO bodies (Name, StartYear, EndYear, Image, BodyOrder, BodyCatID, HeaderImage, handle, slug, platform_group_id)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [
      data.name || "",
      data.startYear || "0",
      ...
      data.image || "0",
      data.bodyOrder ?? 0,
      data.platformGroupId ?? data.bodyCatId ?? "0",
      data.headerImage || "0",
      data.handle || null,
      data.slug || null,
      data.platformGroupId ?? null,
    ],
  );
  return r.insertId;
}

export async function updateBodyAdmin(bodyId, data) {
  const [r] = await pool.query(
    `UPDATE bodies SET Name = ?, StartYear = ?, EndYear = ?, BodyOrder = ?, slug = ?, platform_group_id = ?
     WHERE BodyID = ?`,
    [
      data.name ?? "",
      data.startYear ?? "0",
...
export async function deleteBodyAdmin(bodyId) {
  const [r] = await pool.query("DELETE FROM bodies WHERE BodyID = ?", [bodyId]);
  return r.affectedRows > 0;
}
