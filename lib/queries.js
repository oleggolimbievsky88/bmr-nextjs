...
/** Send dealer PO (status -> sent, set sent_at). Optional notes saved with the PO. */
export async function sendDealerPO(poId, customerId, notes = null) {
...
export async function getAdminDealerPOsPaginated(
  ...
  if (customer && String(customer).trim()) {
    ...
    conditions.push("(c.firstname LIKE ? OR c.lastname LIKE ? OR c.email LIKE ?)");
  ...
  if (dateFrom) {
    conditions.push("(d.sent_at >= ? OR (d.sent_at IS NULL AND d.created_at >= ?))");
  ...
  const orderCol = DEALER_PO_SORT_COLUMNS[sortColumn] || DEALER_PO_SORT_COLUMNS.sent_at;
...
export async function getMenuData() {
  try {
    ...
    try {
      ...
      console.warn("Menu: platforms query failed, falling back to bodies:", err.message);
    }
    if (rows.length === 0) {
      try {
    ...
    const gmLateBodies = rows.filter((body) => {
      ...
      return c.includes("gm late model") || (c.includes("late model") && c.includes("gm"));
    });
    ...
    function getPlatformSlug(body) {
      const nameSlug = (body.Name || "")
        ...
        .replace(/\//g, "-")
        .replace(/[^a-z0-9-]/g, "");
      const computed = `${body.StartYear}-${body.EndYear}-${nameSlug}`.replace(/-+/g, "-");
...
export async function getPlatformBySlug(slug) {
  ...
  try {
    ...
    for (const row of platformRows) {
      const rowSlug = (row.slug && row.slug.trim()) || buildPlatformSlugFromRow(row);
