...
export async function getFilteredProductsPaginated({
  ...
  let query = "SELECT * FROM products WHERE Display = 1 AND EndProduct != 1";
  const params = [];
  ...
  if (color) {
    // color can be comma-separated, so use IN (...)
    const colorArr = color
      .split(",")
      .map((c) => c.trim())
      .filter(Boolean);
    if (colorArr.length > 0) {
      query += ` AND Color IN (${colorArr.map(() => "?").join(",")})`;
      params.push(...colorArr);
    }
  }
  if (brand) {
    // brand can be comma-separated, so use IN (...)
    const brandArr = brand
      .split(",")
      .map((b) => b.trim())
      .filter(Boolean);
    if (brandArr.length > 0) {
      query += ` AND ManName IN (${brandArr.map(() => "?").join(",")})`;
