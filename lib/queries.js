...
export async function getProductById(id) {
  const [rows] = await pool.query(
    `SELECT p.*, b.Name as PlatformName FROM products p
    JOIN bodies b ON p.BodyID = b.BodyID
    WHERE p.ProductID = ? AND p.Display = '1' AND p.EndProduct != 1`,
    [id]
  );
...
export async function getPlatformBySlug(slug) {
  const query = `
    SELECT 
      ...
      StartYear as startYear,
      EndYear as endYear,
      Image as platformImage,  -- Ensure this column exists!
      HeaderImage as headerImage,
      slug
    FROM bodies
    WHERE slug = ?
  ...
  console.log("🔍 Running query for platform:", query, [slug]); // Debugging log
