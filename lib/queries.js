...
export async function validateCouponForCart(
  ...
  try {
    ...
    const lineSubtotals = (cartItems || []).map((item) => {
      ...
      return { lineSub, eligible: eligibleIds.has(Number(item?.ProductID ?? item?.productId)) };
    });
    ...
    const lineItemDiscounts = lineSubtotals.map(({ lineSub, eligible }) => {
      ...
      const amount = Math.round((totalDiscount * (lineSub / eligibleSubtotal)) * 100) / 100;
      return amount;
    });
    ...
    if (currentSum !== totalDiscount && lineItemDiscounts.length > 0) {
      ...
      const lastEligibleIndex = lineSubtotals.map((x, i) => (x.eligible ? i : -1)).filter((i) => i >= 0).pop();
      if (lastEligibleIndex != null) {
        lineItemDiscounts[lastEligibleIndex] = Math.round((lineItemDiscounts[lastEligibleIndex] + diff) * 100) / 100;
      }
    }
...
export async function createOrderItems(orderId, items) {
  try {
    ...
    const insertPromises = items.map(async (item) => {
      const lineDiscount = parseFloat(item.lineDiscount) >= 0 ? parseFloat(item.lineDiscount) : 0;
      const values = [
        orderId,
        item.productId || null,
        item.name,
