...
// Get coupon by code (updated for better validation)
  ...
  try {
    const query = `
      ...
      WHERE CouponCode = ?
        ...
        AND EndDate >= CURDATE()
        AND (StartTime = '0' OR TIME(NOW()) >= StartTime)
        AND (EndTime = '0' OR TIME(NOW()) <= EndTime)
        AND (maxuses = 0 OR currentuses < maxuses)
      ORDER BY
        CASE WHEN Value != '0' AND Value IS NOT NULL THEN 0 ELSE 1 END,
        CouponID DESC
      LIMIT 1
    `;

    console.log("Executing query:", query);
    console.log("With parameters:", [couponCode]);
    const [rows] = await pool.query(query, [couponCode]);
    console.log("Query result for coupon", couponCode, ":", rows);
    return rows[0] || null;
  } catch (error) {
...
export async function validateCouponForCart(
  ...
  try {
    ...
    // Check if coupon has valid value and value type
    // Handle both old table format and new format
    ...
    if (value > 0) {
      if (
        ...
        // For old table, assume percentage if valueType is "0" but value > 0
        const subtotal = cartItems.reduce(
          (sum, item) => sum + parseFloat(item.Price) * item.quantity,
          0
        );
        discountAmount = (subtotal * value) / 100;
      } else if (valueType === "fixed" || valueType === "$") {
        discountAmount = value;
      }
    }

    if (coupon.freecshipping === "1") {
      freeShipping = true;
    }
...
export async function recordCouponUsage(
  ...
  try {
    ...
    await pool.query(
      "UPDATE coupons SET currentuses = currentuses + 1 WHERE CouponID = ?",
      [couponId]
    );

    // Record in coupon history
    await pool.query(
      `
      INSERT INTO couponhistory
      (CouponID, CustomerID, InvoiceID, Value, domhandling)
      VALUES (?, ?, ?, ?, 0)
    `,
      [couponId, customerId, invoiceId, discountAmount]
    );
