...
export async function getVehiclesForSearch() {
  ...
  try {
    const [r] = await pool.query(
      `SELECT v.VehicleID, v.StartYear, v.EndYear, v.Make, v.Model, v.SubModel, v.BodyID,
              p.slug AS p_slug, p.Name AS p_Name, p.StartYear AS p_StartYear, p.EndYear AS p_EndYear,
              b.slug AS b_slug, b.Name AS b_Name, b.StartYear AS b_StartYear, b.EndYear AS b_EndYear
       FROM vehicles v
       LEFT JOIN platforms p ON p.PlatformID = CAST(v.BodyID AS UNSIGNED)
       LEFT JOIN bodies b ON b.BodyID = CAST(v.BodyID AS UNSIGNED)
    ...
    if (
      ...
      const [r] = await pool.query(
        `SELECT v.VehicleID, v.StartYear, v.EndYear, v.Make, v.Model, v.BodyID,
                p.slug AS p_slug, p.Name AS p_Name, p.StartYear AS p_StartYear, p.EndYear AS p_EndYear,
                b.slug AS b_slug, b.Name AS b_Name, b.StartYear AS b_StartYear, b.EndYear AS b_EndYear
         FROM vehicles v
         LEFT JOIN platforms p ON p.PlatformID = CAST(v.BodyID AS UNSIGNED)
      ...
      rows = r || [];
    } else if (e.code === "ER_NO_SUCH_TABLE" && e.message?.includes("bodies")) {
      try {
        const [r2] = await pool.query(
          `SELECT v.VehicleID, v.StartYear, v.EndYear, v.Make, v.Model, v.SubModel,
                  p.slug AS p_slug, p.Name AS p_Name, p.StartYear AS p_StartYear, p.EndYear AS p_EndYear
           FROM vehicles v
           INNER JOIN platforms p ON p.PlatformID = CAST(v.BodyID AS UNSIGNED)
           ORDER BY v.StartYear DESC, v.Make, v.Model`,
        );
        rows = r2 || [];
      } catch (e2) {
        if (e2.message?.includes("SubModel")) {
          const [r3] = await pool.query(
            `SELECT v.VehicleID, v.StartYear, v.EndYear, v.Make, v.Model,
                    p.slug AS p_slug, p.Name AS p_Name, p.StartYear AS p_StartYear, p.EndYear AS p_EndYear
             FROM vehicles v
             INNER JOIN platforms p ON p.PlatformID = CAST(v.BodyID AS UNSIGNED)
             ORDER BY v.StartYear DESC, v.Make, v.Model`,
  ...
  // Load all platforms (and bodies) for fallback lookup when vehicle BodyID has no direct match
  let platformsForFallback = [];
  let bodiesForFallback = [];
  try {
  ...
  try {
    const [bRows] = await pool.query(
      "SELECT BodyID, Name, StartYear, EndYear, slug FROM bodies",
    );
    bodiesForFallback = bRows || [];
  } catch (_) {}
  ...
  for (const r of rows) {
    let slug =
      (r.p_slug && r.p_slug.trim()) || (r.b_slug && r.b_slug.trim()) || "";
    if (!slug) {
      const plat =
        r.p_Name != null
    ...
    if (!slug) {
      // Fallback: find platform/body by model name and overlapping year range
      ...
      const sources = [
        ...platformsForFallback.map((row) => ({
          Name: row.Name,
          StartYear: row.StartYear,
          EndYear: row.EndYear,
          slug: row.slug,
        })),
        ...bodiesForFallback.map((row) => ({
          Name: row.Name,
          StartYear: row.StartYear,
          EndYear: row.EndYear,
          slug: row.slug,
...
export async function validateCouponForCart(
  ...
  try {
    ...
    // Gift cards: apply to full cart up to remaining balance (store credit)
    ...
    if (isGiftCard && remainingBalance > 0) {
      ...
      const lineItemDiscounts = (cartItems || []).map((item) => {
        const lineSub = parseFloat(item.Price || 0) * (item.quantity || 1);
        if (lineSub <= 0 || cartTotal <= 0) return 0;
        return Math.round(totalDiscount * (lineSub / cartTotal) * 100) / 100;
      });
      // Fix rounding
