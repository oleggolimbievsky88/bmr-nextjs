...
export async function getAttributeFiltersForCategory({
  ...
  const sql = `
    SELECT ca.label AS attributeLabel, ca.slug AS attributeSlug, ca.sort_order,
           pav.value AS value, COUNT(DISTINCT p.ProductID) AS count
    FROM products p
    ...
    ${whereClause}
    GROUP BY ca.id, ca.label, ca.slug, ca.sort_order, pav.value
    ORDER BY ca.sort_order, ca.label, pav.value
  `;
  const [rows] = await pool.query(sql, params);
  const bySlug = {};
  for (const r of rows || []) {
...
export async function getProductAttributeValues(productId) {
  try {
    const [rows] = await pool.query(
      `SELECT ca.id AS category_attribute_id, ca.slug, ca.label, ca.type, pav.value
