    ...
    if (mainCategories.length > 0) {
      ...
      for (const mainCat of mainCategories) {
        ...
        const categoriesQuery = `
          ...
          FROM categories c
          LEFT JOIN products p ON FIND_IN_SET(c.CatID, p.CatID) AND p.Display = 1
      ...
      const generalCategoriesQuery = `
        ...
        FROM categories c
        JOIN products p ON FIND_IN_SET(c.CatID, p.CatID) AND p.Display = 1
          AND (EXISTS (SELECT 1 FROM product_platforms pp WHERE pp.ProductID = p.ProductID AND pp.BodyID = ?) OR p.BodyID = ?)
...
export async function getMainCategories(platformSlug) {
  ...
  `;
  const [rows] = await pool.query(query, [platform.id, platform.id]);
  return rows;
}
...
// Admin: Get product by ID (including hidden); attaches BodyIDs and platforms from product_platforms
