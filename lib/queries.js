 ...
 * (manufacturer name contains "BMR"). Empty list returns false.
...
export async function areAllProductsBmr(productIds) {
  ...
  try {
    const [rows] = await pool.query(
      `SELECT COUNT(*) AS bmr_count
       FROM products p
       INNER JOIN mans m ON p.ManID = m.ManID
       WHERE p.ProductID IN (?) AND UPPER(m.ManName) LIKE '%BMR%'`,
      [productIds],
    );
    const bmrCount = rows?.[0]?.bmr_count ?? 0;
    return bmrCount === productIds.length;
  } catch (error) {
