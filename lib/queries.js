    ...
    for (const category of categories) {
      ...
      const processedBodies = await Promise.all(
        bodies.map(async (body) => {
          ...
          const yearPrefix = `${body.StartYear}-${body.EndYear}`;
          const platformSlug = body.Name.toLowerCase().replace(/\s+/g, "-");

          return {
            heading: `${yearPrefix} ${body.Name}`,
            links: mainCategories.map((cat) => ({
              href: `/platform/${yearPrefix}-${platformSlug}/${cat.MainCatName.toLowerCase().replace(
                /\s+/g,
                "-"
              )}`,
...
export async function getProductTypes(platformName, categoryId = null) {
  let query = `
    SELECT DISTINCT pt.TypeID as id, pt.TypeName as name, pt.Image as image
    FROM producttypes pt
    JOIN products p ON p.TypeID = pt.TypeID
    JOIN bodies b ON p.BodyID = b.BodyID
    WHERE b.Name = ?
    AND p.Display = "1"
  `;
