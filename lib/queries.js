...
export async function getFordPlatformsWithVehicles() {
  try {
    ...
    const platforms = rows.map((row) => {
      ...
      const imageUrl = row.image && row.image !== "0"
...
export async function getGMPlatformsWithVehicles() {
  try {
    ...
    const [rows] = await pool.query(query);
    return rows.map((row) => {
      const slug = row.slug || `${row.startYear}-${row.endYear}-${row.name}`.toLowerCase().replace(/\s+/g, "-");
      const imageUrl = row.image && row.image !== "0"
        ? `/images/cars/${row.image}`
        : `/images/cars/2024-2024 Mustang.png`;
...
export async function getNewProducts(scratchDent, limit = 35) {
  const query = `
    ...
    WHERE p.Display = 1 AND p.EndProduct = 0 AND p.NewPartDate != "0" AND (NewPart = 0 OR p.BlemProduct = ?) AND p.BlemProduct = ?
