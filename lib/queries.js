...
export async function getMenuData() {
  try {
    ...
    // Format slug in a standard way for all entries
    const formatSlug = (body) => {
      // If a slug exists and doesn't have dashes, add them
      if (body.slug) {
        return body.slug;
      }
    ...
    // Create the menu data structure with reliable slugs
    const menuData = {
...
export async function getMainCategories(platformSlug) {
  try {
    // First get the platform using our flexible lookup function
    const platform = await getPlatformBySlug(platformSlug);

    if (!platform) {
      console.error(`Platform not found for slug: ${platformSlug}`);
      return [];
    }
    ...
    console.log("Main Categories Query:", query, [bodyId]);
...
export async function getCategoriesByPlatform(
  ...
  try {
    // First get platform data using our more flexible lookup function
    const platform = await getPlatformBySlug(platformSlug);
    ...
    // Extract platform info for consistency
    const platformInfo = {
      headerImage: platform.headerImage
        ? `https://bmrsuspension.com/siteart/header/${platform.headerImage}`
        : null,
      name: platform.name,
    ...
    console.error("Error in getCategoriesByPlatform:", error);
    throw error;
  }
}
...
export async function getPlatformBySlug(slug) {
  try {
    console.log("Looking up platform with slug:", slug);
    ...
    let [results] = await pool.query(query, [slug]);

    // If no results, try to match by constructing the slug from name and years
    if (!results || results.length === 0) {
      console.log("No exact match found, trying constructed slug match...");

      // Try to extract name and years from the slug
      // Format could be like "corvette-1997-2004" or "1997-2004-corvette"
      const parts = slug.split("-");

      // Try with name first format (e.g., corvette-1997-2004)
      if (parts.length >= 3) {
        // Check if last two parts might be years
        const possibleEndYear = parts.pop();
        const possibleStartYear = parts.pop();
        const possibleName = parts.join(" ");

        // Check if they could be valid years
        if (
          /^\d{4}$/.test(possibleStartYear) &&
          /^\d{4}$/.test(possibleEndYear)
        ) {
          console.log(
            `Trying to match name: "${possibleName}", startYear: ${possibleStartYear}, endYear: ${possibleEndYear}`
          );
          ...
          query = `
            ...
            FROM bodies b
            JOIN bodycats bc ON b.BodyCatID = bc.BodyCatID
            WHERE LOWER(b.Name) LIKE ? AND b.StartYear = ? AND b.EndYear = ?
            LIMIT 1
          `;
      ...
      // If still no results, try with years first format (e.g., 1997-2004-corvette)
      if (!results || results.length === 0) {
        const parts = slug.split("-");
        if (parts.length >= 3) {
          // First two parts might be years, rest is the name
          const possibleStartYear = parts[0];
          const possibleEndYear = parts[1];
          ...
          if (
            ...
            console.log(
              `Trying to match with years first: "${possibleName}", startYear: ${possibleStartYear}, endYear: ${possibleEndYear}`
            );
      ...
      // If still no results, try a more flexible name-based search
      if (!results || results.length === 0) {
        console.log("Trying flexible name-based search...");

        // Extract a likely vehicle name from the slug
        const vehicleName = slug
          .split("-")
          .filter((part) => !/^\d{4}$/.test(part))[0];
        ...
        [results] = await pool.query(query, [`%${vehicleName}%`]);
      }
    }
    ...
    console.log("No platform found for slug:", slug);
    return null;
  } catch (error) {
...
export async function getProductsByMainCategory(
  ...
  try {
    ...
    const params = limit
      ? [bodyId, mainCategory, limit]
      : [bodyId, mainCategory];

    console.log("Products by Main Category Query:", query);
    console.log("Products by Main Category Params:", params);
...
export async function getFeaturedProductsByPlatform(platformSlug, limit = 8) {
  try {
    // Get the platform data using our flexible lookup function
    const platform = await getPlatformBySlug(platformSlug);
    ...
    console.log("Featured Products Query:", query, [bodyId, limit]);
    const [products] = await pool.query(query, [bodyId, limit]);
    return products;
  } catch (error) {
    console.error("Error in getFeaturedProductsByPlatform:", error);
    return [];
  }
}
...
export async function getProducts(platform, mainCategory, category = null) {
  try {
    ...
    if (!platformData) {
      console.error(`Platform not found for slug: ${platform}`);
      return [];
    }

    const bodyId = platformData.id;
    ...
    let sql = `
      ...
      AND p.CatID IN (
        ...
        WHERE MainCatID = (
          ...
          WHERE MainCatName = ? AND BodyID = ?
        )
      )
    `;

    const params = [bodyId, formattedMainCategory, bodyId];
