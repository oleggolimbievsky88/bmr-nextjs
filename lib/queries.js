...
export async function getFilteredProductsPaginated({
  ...
  const query = `
    ...
    WHERE p.Display = 1
      AND p.EndProduct != 1
      ${platformId ? "AND p.BodyID = ?" : ""}
      ${mainCategoryId ? "AND c.MainCatID = ?" : ""}
      ${categoryId ? "AND c.CatID = ?" : ""}
  ...
  if (mainCategoryId) params.push(mainCategoryId);
  if (categoryId) params.push(categoryId);
  params.push(Number(limit), Number(offset));

  const [rows] = await pool.query(query, params);
  return rows; // Should be [] when no more products
}
