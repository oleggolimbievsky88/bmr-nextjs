  ...
  try {
    ...
    if (discountValue > 0) {
      if (discountType === "percentage") {
        ...
        console.log(`Calculated ${discountValue}% discount: $${discountAmount} from cart total $${cartTotal}`);
        ...
        if (
          ...
          console.log(`Applying max discount limit: $${coupon.max_discount_amount}`);
          discountAmount = parseFloat(coupon.max_discount_amount);
        }
      ...
      console.error(`⚠️ Coupon ${coupon.code} has discount_value of 0.00. The discount will not be applied.`);
      console.error(`Please run the fix script: scripts/fix-coupon-discount-values.sql`);
