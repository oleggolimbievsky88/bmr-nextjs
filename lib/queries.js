...
export async function getCouponsAdminPaginated(limit = 25, offset = 0) {
  try {
    const [[countRows], [rows]] = await Promise.all([
      pool.query("SELECT COUNT(*) AS total FROM coupons_new"),
      pool.query(
        `SELECT * FROM coupons_new ORDER BY created_at DESC LIMIT ? OFFSET ?`,
