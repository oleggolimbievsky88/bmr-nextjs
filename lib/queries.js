...
export async function searchAllQuick(searchQuery, limits = {}) {
  ...
  const [products] = await pool.query(
    `
      ...
      LIMIT ?
    `,
    [like, like, like, like, starts, starts, l.products]
  );

  // categories (sub-categories)
  const [categories] = await pool.query(
    `
      SELECT c.CatID, c.CatName, c.CatSlug, c.CatImage
      FROM categories c
      WHERE c.CatName LIKE ?
      ORDER BY CASE WHEN c.CatName LIKE ? THEN 1 ELSE 2 END, c.CatName
      LIMIT ?
    `,
    [like, starts, l.categories]
  );
  ...
  const [vehicles] = await pool.query(
    `
      ...
      FROM vehicles
      WHERE Make LIKE ? OR Model LIKE ? OR CONCAT(StartYear, '-', EndYear) LIKE ?
      ORDER BY CASE WHEN Make LIKE ? THEN 1 ELSE 2 END, StartYear DESC
      LIMIT ?
    `,
    [like, like, like, starts, l.vehicles]
  );
