...
export function getProductImageUrl(imagePath) {
  ...
  // Rewrite legacy.bmrsuspension.com URLs to use current base (domain is dead)
  if (
    typeof imagePath === "string" &&
    imagePath.startsWith("http") &&
    imagePath.includes("legacy.bmrsuspension.com")
  ) {
    const match = imagePath.match(/\/siteart\/products\/(.+)$/);
    if (match) {
      return `${PRODUCT_IMAGE_BASE}/${match[1]}`;
    }
    const parts = imagePath.split("/");
    const filename = parts[parts.length - 1];
    if (filename) return `${PRODUCT_IMAGE_BASE}/${filename}`;
  }

  // If it's already a full URL (and not legacy), return as is
...
export function getPlatformImageUrl(imagePath) {
  ...
  if (
    ...
    const match = imagePath.match(/\/siteart\/cars\/(.+)$/);
    if (match) return `${PLATFORM_IMAGE_BASE}/${encodeURI(match[1])}`;
    const parts = imagePath.split("/");
    const filename = parts[parts.length - 1];
    if (filename) return `${PLATFORM_IMAGE_BASE}/${encodeURI(filename)}`;
  }
  if (typeof imagePath === "string" && imagePath.startsWith("http")) {
    return imagePath;
  }
...
export function getPlatformBannerUrl(imagePath) {
  ...
  if (
    ...
    const match = imagePath.match(/\/siteart\/platformHeaders\/(.+)$/);
    if (match) return `${PLATFORM_BANNER_BASE}/${encodeURI(match[1])}`;
    const parts = imagePath.split("/");
    const filename = parts[parts.length - 1];
    if (filename) return `${PLATFORM_BANNER_BASE}/${encodeURI(filename)}`;
  }
  if (typeof imagePath === "string" && imagePath.startsWith("http")) {
    return imagePath;
  }
...
export function getCategoryImageUrl(imagePath) {
  ...
  if (
    ...
    const match = imagePath.match(/\/siteart\/categories\/(.+)$/);
    if (match) return `${CATEGORY_IMAGE_BASE}/${match[1]}`;
    const parts = imagePath.split("/");
    const filename = parts[parts.length - 1];
    if (filename) return `${CATEGORY_IMAGE_BASE}/${filename}`;
  }
  if (typeof imagePath === "string" && imagePath.startsWith("http")) {
    return imagePath;
  }
