...
// Path under assets origin for vehicle thumbnails (e.g. "siteart/cars" for BMR, "cars" for Control Freak).
const PLATFORM_IMAGE_PATH =
  (typeof process.env.NEXT_PUBLIC_PLATFORM_IMAGE_PATH === "string" &&
    process.env.NEXT_PUBLIC_PLATFORM_IMAGE_PATH.trim()) ||
  "siteart/cars";
const PLATFORM_BANNER_PATH =
...
function getPlatformImageBase() {
  ...
  // Vehicle thumbnails live on R2; use assets domain when set (SITE_URL is the app, not the bucket).
  const assetsBase =
    typeof process.env.NEXT_PUBLIC_ASSETS_BASE_URL === "string" &&
    process.env.NEXT_PUBLIC_ASSETS_BASE_URL?.trim();
  const site =
  ...
  const path = PLATFORM_IMAGE_PATH.replace(/^\//, "");
...
function getPlatformBannerBase() {
  ...
  const site =
    ...
    process.env.NEXT_PUBLIC_SITE_URL?.trim();
  const origin = withoutWww((assetsBase || site || "").replace(/\/$/, ""));
  const path = PLATFORM_BANNER_PATH.replace(/^\//, "");
  return origin ? `${origin}/${path}` : `/${path}`;
}
...
export function getCategoryImageUrl(imagePath) {
  ...
  // Plain filename (no path): admin uploads live under /images/categories/
  if (typeof imagePath === "string" && !imagePath.includes("/")) {
    return `/images/categories/${encodeURIComponent(imagePath)}`;
  }
