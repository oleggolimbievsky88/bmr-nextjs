...
const PRODUCT_IMAGE_BASE =
  ...
  (PRODUCT_BRAND_KEY === "controlfreak"
    ? "/images/products"
    : "/siteart/products");
...
/** Base URL for admin-uploaded assets (e.g. Shop by Make logos in R2). When set, used to fix links that point at the main site instead of the bucket. */
const UPLOADS_BASE_URL =
  (typeof process.env.NEXT_PUBLIC_UPLOADS_BASE_URL === "string" &&
    process.env.NEXT_PUBLIC_UPLOADS_BASE_URL?.trim()) ||
  "";

/**
 * Resolve Shop by Make image URL. When R2 is used, uploads may have been stored with the main site URL by mistake;
 * if UPLOADS_BASE_URL or ASSETS_BASE_URL is set and points to the bucket, rewrite so the image loads.
 * For BMR: logos live at https://assets.bmrsuspension.com/images/logo/; we rewrite bmrsuspension.com URLs there.
 * If the DB already has an assets-domain URL (e.g. assets.bmrsuspension.com), return it as-is — do not overwrite with base.
 */
const BMR_ASSETS_ORIGIN = "https://assets.bmrsuspension.com";

export function getShopByMakeImageUrl(imagePath) {
  if (!imagePath || imagePath === "0") return "";
  let pathname = "";
  let storedHost = "";
  if (typeof imagePath === "string" && imagePath.startsWith("http")) {
    try {
      ...
      pathname = u.pathname;
      storedHost = u.hostname.toLowerCase();
      // Already an assets-domain URL (e.g. assets.bmrsuspension.com) — use as-is so we never overwrite correct URLs
      if (storedHost.includes("assets.")) {
        return withoutWww(imagePath);
      }
    } catch {
      return withoutWww(imagePath);
  ...
  const base =
    UPLOADS_BASE_URL ||
    (ASSETS_BASE_URL && ASSETS_BASE_URL.startsWith("http")
      ? ASSETS_BASE_URL.replace(/\/$/, "")
      : "");
  if (pathname.startsWith("/images/logo/")) {
    if (base) {
      return `${base}${pathname}`;
    }
    // BMR: DB often has bmrsuspension.com URLs but files are on R2 at assets.bmrsuspension.com
    if (
      storedHost === "bmrsuspension.com" ||
      storedHost === "www.bmrsuspension.com"
    ) {
      return `${BMR_ASSETS_ORIGIN}${pathname}`;
    }
  }
  return typeof imagePath === "string" && imagePath.startsWith("http")
    ? withoutWww(imagePath)
    : imagePath;
}
