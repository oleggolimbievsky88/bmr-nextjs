-- Brand core database: shared brand config across multiple sites.
-- Run once per server: mysql -u user -p < database/brand_core.sql
-- Then set DATABASE_BRAND_CORE_URL=mysql://user:password@host:3306/brand_core in each app's .env

CREATE DATABASE IF NOT EXISTS `brand_core` DEFAULT CHARACTER SET utf8mb4;
USE `brand_core`;
...
INSERT INTO `brands` (
  ...
  `logo`, `contact`, `social`, `assurance_bar_items`, `about_brand`, `same_as`, `shop_by_make`, `nav_labels`, `nav_urls`, `nav_order`
) VALUES (
